#!/usr/bin/env bash
# sc-rns-bridge launcher for macOS.
# Run: ./run.command   (double-click in Finder works too)
#
# This is the same launcher as run.sh; macOS just expects the .command
# extension for double-clickable terminal scripts.
set -euo pipefail
...
find_svends() {
    local candidates=(
        "$HOME/Library/Application Support/Steam/steamapps/common/Sven Co-op/svends_run"
    ...
    local found
    found=$(find "$HOME/Library/Application Support/Steam" "$HOME/.local/share/Steam" "$HOME/.steam" \
...
case "$choice" in
    1)
        ...
        if [ -z "$SVENDS" ]; then
            ...
            echo "installed elsewhere, enter the full path to svends_run."
            echo
