...
export type BrowserRendezvousId = string & {
  readonly [rendezvousIdBrand]: "BrowserRendezvousId";
};

export type AutoWifiFailure =
  | HostApiUnavailable<"WebSocket" | "Crypto" | "LocalStorage" | "Fetch">
  | PermissionDenied<"auto-wifi">
  | AlreadyActive<"auto-wifi">
  | Tag<"SelectionIdentityUnavailable", { readonly detail: string }>
  | Tag<"DiscoveryFailed", { readonly detail: string }>
  | RuntimeRejected;

export type AutoWifiGatewayStatus = {
...
export type AutoWifiControllerStatus =
  | Tag<"Starting">
  | Tag<"Discovering", { readonly attempt: number }>
  | Tag<"Active", { readonly gateways: readonly AutoWifiGatewayStatus[] }>
  | Tag<"Unavailable", AutoWifiFailure>
  | Tag<"Closed">;

export type AutoWifiRegistrationOutcome =
  | Tag<"Registered", InterfaceId>
  | AlreadyActive<"auto-wifi">
  | RuntimeRejected;

export type AutoWifiControllerCloseOutcome = Tag<"Closed"> | RuntimeRejected;
...
const SELECTION_DOMAIN = new TextEncoder().encode(
  "prns browser gateway selection v1",
);
...
const CATALOG_PROBES = [
  ...
  },
  {
    ...
    targetAddressSpace: "local" as const,
  },
] as const;

type ProbeFailure = AutoWifiFailure;

type ProbedGateway = {
  readonly id: BrowserRendezvousId;
  readonly idBytes: Uint8Array;
  readonly url: string;
  readonly pending: PendingGatewaySocket;
  readonly localhost: boolean;
};
...
export type BrowserGatewayCatalogOutcome =
  | Tag<"Discovered", readonly BrowserGatewayCatalogEntry[]>
  | Tag<"Failed", ProbeFailure>;

type CatalogGateway = BrowserGatewayCatalogEntry;
type CatalogOutcome = BrowserGatewayCatalogOutcome;

type KnownGateway = BrowserGatewayCatalogEntry & {
...
type GatewayCandidate = {
  ...
  readonly url: string;
  readonly localhost: boolean;
  readonly session?: AutoWifiGatewaySession;
  readonly probe?: ProbedGateway;
};

type RankedGatewayCandidate = GatewayCandidate & {
...
type GatewayWeightOutcome =
  | Tag<"Weighted", string>
  | Extract<AutoWifiFailure, Tag<"SelectionIdentityUnavailable", unknown>>;

type AutoWifiAttachOutcome = Tag<"Attached"> | AutoWifiFailure;

type ServerHelloDecodeOutcome =
  | Tag<
      "Decoded",
      { readonly id: BrowserRendezvousId; readonly idBytes: Uint8Array }
    >
  | Tag<"MalformedHello", { readonly detail: string }>;

type RendezvousIdParseOutcome =
  | Tag<"Parsed", BrowserRendezvousId>
  | Tag<"Invalid">;
...
export class AutoWifiController {
  ...
  async #refresh(): Promise<void> {
    ...
    try {
      ...
      const catalogPromise = Promise.all(
        CATALOG_PROBES.map((probe) => fetchCatalog(probe)),
      );
      ...
      const candidates = new Map<BrowserRendezvousId, GatewayCandidate>();
      ...
      const ranked: RankedGatewayCandidate[] = [];
      ...
      const selected = new Set<BrowserRendezvousId>();
  ...
  #attach(probe: ProbedGateway): AutoWifiAttachOutcome {
  ...
  #sessionClosed(
    ...
    session: AutoWifiGatewaySession,
    outcome: AutoWifiDetachOutcome,
  ): void {
...
type PendingGatewayClaim = {
  readonly socket: WebSocket;
  readonly frames: readonly unknown[];
};

type PendingGatewayClaimOutcome =
  | Tag<"Claimed", PendingGatewayClaim>
  | Tag<"Unavailable">;

type WebSocketPayloadLengthOutcome =
  | Tag<"Measured", number>
  | Tag<"UnsupportedFrame">;
...
class PendingGatewaySocket {
  ...
  constructor(socket: WebSocket, frameCap: number) {
    this.#socket = socket;
  ...
  claim(): PendingGatewayClaimOutcome {
    if (
      this.#closed ||
      this.#claimed ||
...
class AutoWifiGatewaySession {
  ...
  readonly #initialFrames: readonly unknown[];
  readonly #onClosed: (outcome: AutoWifiDetachOutcome) => void;
  readonly interfaceId: InterfaceId;
  readonly id: BrowserRendezvousId;
  readonly url: string;
  readonly localhost: boolean;
  #lifecycle: Tag<"Open"> | Tag<"Closed", AutoWifiDetachOutcome> = Tag("Open");
  #released = false;
  #writeQueue: Promise<void> = Promise.resolve();
  #readQueue: Promise<void> = Promise.resolve();

  constructor(
    host: AutoWifiRuntimeHost,
    socket: WebSocket,
    ...
    url: string,
    localhost: boolean,
    onClosed: (outcome: AutoWifiDetachOutcome) => void,
  ) {
    this.#host = host;
    this.#socket = socket;
  ...
  snapshot(): AutoWifiGatewayStatus {
    return {
      id: this.id,
      url: this.url,
  ...
  async close(): Promise<AutoWifiDetachOutcome> {
  ...
  #queueInbound(value: unknown): void {
  ...
  #release(outcome: AutoWifiDetachOutcome): void {
    if (!this.#released) {
      this.#released = true;
...
async function probeGateway(
  ...
  return new Promise((resolve) => {
    let phase: "opening" | "hello" | "decoding" = "opening";
    let settled = false;
    ...
    const settle = (outcome: GatewayProbeOutcome): void => {
      if (settled) {
        return;
      }
      settled = true;
    ...
    const handleOpen = (): void => {
      if (socket.protocol !== SUBPROTOCOL) {
        settle(
          Tag("Failed", Tag("DiscoveryFailed", {
...
async function fetchCatalog(
  probe: (typeof CATALOG_PROBES)[number],
  ...
  try {
    ...
    } as RequestInit & { targetAddressSpace: "loopback" | "local" });
...
async function readCappedBody(
  response: Response,
): Promise<Tag<"Read", Uint8Array> | Tag<"Failed", ProbeFailure>> {
  ...
  if (declared !== null) {
    ...
    if (!Number.isSafeInteger(length) || length < 0 || length > MAX_CATALOG_BODY_BYTES) {
      return Tag(
        "Failed",
        Tag("DiscoveryFailed", { detail: "catalog Content-Length exceeds the response cap" }),
      );
    }
  }
  if (!response.body) {
  ...
  try {
    while (true) {
      ...
      if (length > MAX_CATALOG_BODY_BYTES) {
        ...
        return Tag(
          "Failed",
          Tag("DiscoveryFailed", { detail: "catalog body exceeds the response cap" }),
        );
      }
...
export function parseBrowserGatewayCatalog(
  bytes: Uint8Array,
): BrowserGatewayCatalogOutcome {
  return parseCatalog(bytes);
}

function parseCatalog(bytes: Uint8Array): CatalogOutcome {
  ...
  if (!exactRecord(value, ["gateways", "version"]) || value.version !== 1) {
    return Tag(
      "Failed",
      Tag("DiscoveryFailed", { detail: "catalog must be an exact version 1 object" }),
    );
  }
  if (!Array.isArray(value.gateways) || value.gateways.length > MAX_CATALOG_GATEWAYS) {
    return Tag(
      "Failed",
      Tag("DiscoveryFailed", { detail: "catalog gateways must be a bounded array" }),
    );
  }
  const ids = new Set<BrowserRendezvousId>();
  const gateways: CatalogGateway[] = [];
  for (const raw of value.gateways) {
    if (!exactRecord(raw, ["id", "url"]) || typeof raw.id !== "string" || typeof raw.url !== "string") {
      return Tag(
        "Failed",
        Tag("DiscoveryFailed", { detail: "catalog gateway has an invalid shape" }),
      );
    }
    ...
    if (id.tag !== "Parsed" || url.tag !== "Valid" || ids.has(id.data)) {
      return Tag(
        "Failed",
        Tag("DiscoveryFailed", { detail: "catalog gateway ID or URL is invalid or duplicated" }),
      );
    }
    ids.add(id.data);
...
export function validateBrowserGatewayUrl(
  value: string,
): Tag<"Valid", string> | Tag<"Invalid"> {
...
function validateGatewayUrl(
  value: string,
): Tag<"Valid", string> | Tag<"Invalid", ProbeFailure> {
  let url: URL;
  try {
    url = new URL(value);
  } catch (error) {
  ...
  if (
    ...
    return Tag(
      "Invalid",
      Tag("DiscoveryFailed", { detail: `invalid local gateway URL ${value}` }),
    );
  }
  return Tag("Valid", url.toString());
}

function isPermittedHost(hostname: string): boolean {
...
async function gatewayWeight(
  seed: Uint8Array,
  id: BrowserRendezvousId,
): Promise<GatewayWeightOutcome> {
...
function compareCandidates(
  left: RankedGatewayCandidate,
  right: RankedGatewayCandidate,
): number {
  if (left.localhost !== right.localhost) {
    return left.localhost ? -1 : 1;
  }
...
function decodeServerHello(
  ...
  const version =
    ((bytes[CLIENT_HELLO_LENGTH - 2] ?? 0) << 8) |
...
async function websocketBytes(
  value: unknown,
  cap: number,
): Promise<AutoWifiWebSocketDecodeOutcome> {
...
function parseRendezvousId(value: string): RendezvousIdParseOutcome {
  return value.length === ID_HEX_LENGTH && /^[0-9a-f]{32}$/.test(value)
    ? Tag("Parsed", value as BrowserRendezvousId)
    : Tag("Invalid");
}
...
function browserNetworkFailure(error: unknown): ProbeFailure {
  if (
...
function preferredFailure(failures: readonly AutoWifiFailure[]): AutoWifiFailure {
  return (
    failures.find((failure) => failure.tag === "PermissionDenied") ??
...
function closeProbes(outcomes: readonly DirectProbeOutcome[]): void {
...
function websocketPayloadLength(value: unknown): WebSocketPayloadLengthOutcome {
  if (value instanceof ArrayBuffer) {
