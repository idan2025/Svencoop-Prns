...
export type UsbReadOutcome =
  | Tag<"Read", Uint8Array | undefined>
  | InterfaceSessionFailure;

export type WebUsbOpenOutcome =
...
export type UsbStageOutcome<Value> =
  | Tag<"Completed", Value>
  | PermissionDenied<"usb-auto">
  | Cancelled<"usb-auto">
  | ConnectionFailed<"usb-auto">;
...
export class WebUsbAutoTransport {
  ...
  async write(bytes: Uint8Array): Promise<UsbAutoWriteOutcome> {
...
export async function usbStage<T>(
