...
fn braille_lines(grid: &[Vec<Option<Rgb>>], colorize: bool) -> Vec<String> {
...
pub fn print(subtitle: &str) {
    ...
    if mode == RenderMode::Plain {
        println!("{NAME} - {subtitle}");
        println!();
        return;
    }

    println!();
    for line in braille_lines(&canvas(), mode == RenderMode::Color) {
        println!("  {line}");
    }
    println!();
    if mode == RenderMode::Color {
        println!("  {}{subtitle}{RESET}", foreground(SOFT));
    } else {
        println!("  {subtitle}");
    }
    println!();
