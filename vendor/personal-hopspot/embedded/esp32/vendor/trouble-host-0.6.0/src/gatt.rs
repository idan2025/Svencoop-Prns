...
use crate::att::{
    self, Att, AttCfm, AttClient, AttCmd, AttErrorCode, AttReq, AttRsp, AttServer, AttUns,
    ATT_HANDLE_VALUE_IND, ATT_HANDLE_VALUE_NTF,
};
...
impl<P: PacketPool> Drop for GattConnection<'_, '_, P> {
    fn drop(&mut self) {
        trace!(
            "[gatt {}] disconnecting from server",
            self.connection.handle().raw()
        );
...
impl<'stack, 'server, P: PacketPool> GattEvent<'stack, 'server, P> {
    ...
    pub fn new(data: GattData<'stack, P>, server: &'server dyn DynamicAttributeServer<P>) -> Self {
        ...
        let allowed = match &att {
            AttClient::Command(AttCmd::Write { handle, .. }) => {
            ...
            AttClient::Request(req) => match req {
                ...
                AttReq::Read { handle } | AttReq::ReadBlob { handle, .. } => {
                    server.can_read(&data.connection, *handle)
                }
                AttReq::ReadMultiple { handles } => {
                    handles.chunks_exact(2).try_for_each(|handle| {
                        server.can_read(
                            &data.connection,
                            u16::from_le_bytes(handle.try_into().unwrap()),
                        )
                    })
                }
                _ => Ok(()),
        ...
        match att {
            ...
            | AttClient::Request(AttReq::ReadBlob { .. }) => {
...
pub(crate) trait Client<'d, E, P: PacketPool> {
    ...
    fn request(
        &self,
        req: AttReq<'_>,
    ) -> impl Future<Output = Result<Response<P::Packet>, BleHostError<E>>>;
    fn command(&self, cmd: AttCmd<'_>) -> impl Future<Output = Result<(), BleHostError<E>>>;
}

impl<'reference, T: Controller, P: PacketPool, const MAX_SERVICES: usize>
    Client<'reference, T::Error, P> for GattClient<'reference, T, P, MAX_SERVICES>
{
    async fn request(
        &self,
        req: AttReq<'_>,
    ) -> Result<Response<P::Packet>, BleHostError<T::Error>> {
...
impl<'reference, T: Controller, P: PacketPool, const MAX_SERVICES: usize>
    GattClient<'reference, T, P, MAX_SERVICES>
{
...
impl<'reference, C: Controller, P: PacketPool, const MAX_SERVICES: usize>
    GattClient<'reference, C, P, MAX_SERVICES>
{
    ...
    pub async fn new(
        ...
        let mut w = WriteCursor::new(buf.as_mut());
        w.write_hci(&l2cap)?;
        w.write(att::Att::Client(att::AttClient::Request(
            att::AttReq::ExchangeMtu {
                mtu: P::MTU as u16 - 4,
            },
        )))?;
    ...
    pub async fn characteristics<const N: usize>(
        ...
        while let Some(characteristic) = iter.next() {
            if characteristic.props.has_cccd() {
                ...
                characteristic.cccd_handle = match self
                    .get_characteristic_cccd(characteristic.handle + 1, end)
                    .await
                {
                        ...
                        } = AttributeData::decode_declaration(item)?
                        {
                            if let Some((start_handle, _)) = found_indicate_or_notify_uuid {
                                return Ok(Characteristic {
                                    handle: start_handle,
                                    cccd_handle: Some(
                                        self.get_characteristic_cccd(start_handle, handle).await?,
                                    ),
                                    props,
                                    phantom: PhantomData,
                                });
                    ...
                    att::AttErrorCode::ATTRIBUTE_NOT_FOUND => match found_indicate_or_notify_uuid {
                        Some((handle, props)) => {
                            return Ok(Characteristic {
                                handle,
                                cccd_handle: Some(
                                    self.get_characteristic_cccd(handle, service.end).await?,
                                ),
                                props,
                                phantom: PhantomData,
                            });
        ...
        loop {
            ...
            match Self::response(response.pdu.as_ref())? {
                AttRsp::ReadBlob { data } => {
                    debug!(
                        "[read_characteristic_long] Blob read returned {} bytes",
                        data.len()
                    );
                    if data.is_empty() {
    ...
    pub async fn confirm_indication(&self) -> Result<(), BleHostError<C::Error>> {
        self.send_att_data(Att::Client(AttClient::Confirmation(
            AttCfm::ConfirmIndication,
        )))
        .await
    }
...
mod tests {
    ...
    fn build_read_by_type_pdu(
        start: u16,
        end: u16,
        uuid: &Uuid,
    ) -> (<DefaultPacketPool as PacketPool>::Packet, usize) {
