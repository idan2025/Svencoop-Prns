...
use bt_hci::cmd::le::{
    LeConnUpdate, LeReadLocalSupportedFeatures, LeReadPhy, LeSetDataLength, LeSetPhy,
};
use bt_hci::cmd::status::ReadRssi;
use bt_hci::controller::{ControllerCmdAsync, ControllerCmdSync};
use bt_hci::param::{
    AddrKind, AllPhys, BdAddr, ConnHandle, DisconnectReason, LeConnRole, PhyKind, PhyMask,
    PhyOptions, Status,
};
        ...
        C: crate::Controller,
    {
            ...
            .await
        {
            Ok(()) => {
                ...
                stack
                    .host
                    .send_conn_param_update_res(self.handle, &param)
                    .await
            }
            ...
            .await
        {
            Ok(()) => {
                if self.l2cap {
                    ...
                    let timeout: bt_hci::param::Duration<10_000> =
...
impl<'stack, P: PacketPool> Connection<'stack, P> {
    ...
    #[cfg(feature = "gatt")]
    pub(crate) async fn next_gatt_client(&self) -> Result<Pdu<P::Packet>, Error> {
        loop {
            match select(
                self.manager.next_gatt_client(self.index),
                self.manager.next(self.index),
            )
            .await
            {
                Either::First(pdu) => return Ok(pdu),
                Either::Second(ConnectionEvent::Disconnected { .. }) => {
                ...
                Either::Second(
                    ...
                    | ConnectionEvent::RequestConnectionParams(_),
                ) => {}
                #[cfg(feature = "security")]
                Either::Second(
        ...
        T: ControllerCmdAsync<LeConnUpdate> + ControllerCmdSync<LeReadLocalSupportedFeatures>,
    {
        ...
        if self.role() == LeConnRole::Peripheral || cfg!(feature = "connection-params-update") {
            ...
            stack
                .host
                .send_conn_param_update_req(handle, &param)
                .await?;
        }
        Ok(())
    }
