...
impl<'d, P: PacketPool> ConnectionManager<'d, P> {
    ...
    pub(crate) async fn next(&self, index: u8) -> ConnectionEvent {
        poll_fn(|cx| {
            self.with_mut(|state| state.connections[index as usize].events.poll_receive(cx))
        })
        .await
    }
    ...
    pub(crate) async fn next_gatt(&self, index: u8) -> Pdu<P::Packet> {
        poll_fn(|cx| self.with_mut(|state| state.connections[index as usize].gatt.poll_receive(cx)))
            .await
    }
    ...
    pub(crate) fn get_connection_by_peer_address(
        &'d self,
        peer_address: Address,
    ) -> Option<Connection<'d, P>> {
        let mut state = self.state.borrow_mut();
        for (index, storage) in state.connections.iter().enumerate() {
    ...
    pub(crate) fn with_connected_handle<
        F: FnOnce(&mut ConnectionStorage<P::Packet>) -> Result<R, Error>,
        R,
    >(
        &self,
        h: ConnHandle,
    ...
    pub(crate) fn connect(
        ...
        warn!(
            "[link][connect] no available slot found for handle {:?}",
            handle
        );
        Err(Error::NotFound)
    }
                    ...
                    if !peers.is_empty() {
                        for peer in peers.iter() {
                                ...
                                && storage.peer_identity.unwrap().bd_addr == *peer.1
                            {
                                ...
                                debug!(
                                    "[link][poll_accept] connection accepted: state: {:?}",
                                    storage
                                );
    ...
    pub(crate) fn dec_ref(&self, index: u8) {
        self.with_mut(|state| {
            ...
            if conn.refcount == 0 && conn.state == ConnectionState::Connected {
                conn.state =
                    ConnectionState::DisconnectRequest(DisconnectReason::RemoteUserTerminatedConn);
                state.disconnect_waker.wake();
            }
        });
    }
    ...
    pub(crate) fn poll_request_to_send(
        ...
        warn!(
            "[link][pool_request_to_send] connection {:?} not found",
            handle
        );
    ...
    pub(crate) fn exchange_att_mtu(&self, conn: ConnHandle, mtu: u16) -> u16 {
        let mut state = self.state.borrow_mut();
        debug!(
            "exchange_att_mtu: {}, current default: {}",
            mtu, state.default_att_mtu
        );
        let default_att_mtu = state.default_att_mtu;
        for storage in state.connections.iter_mut() {
            match storage.state {
    ...
    pub(crate) fn pass_key_confirm(&self, index: u8, confirm: bool) -> Result<(), Error> {
        #[cfg(feature = "security")]
        {
            if self.state.borrow_mut().connections[index as usize].state
    ...
    pub(crate) fn handle_security_hci_event(
        &self,
        event: bt_hci::event::EventPacket,
    ) -> Result<(), Error> {
        #[cfg(feature = "security")]
        {
    ...
    pub(crate) fn handle_security_hci_le_event(
        &self,
        event: bt_hci::event::le::LeEventPacket,
    ) -> Result<(), Error> {
        #[cfg(feature = "security")]
        {
            ...
            + crate::ControllerCmdSync<bt_hci::cmd::link_control::Disconnect>,
    {
        ...
        match _event {
            ...
            crate::security_manager::SecurityEventData::EnableEncryption(handle, bond_info) => {
                let connection_data = self.state.borrow().connections.iter().enumerate().find_map(
                ...
                if let Some((index, role, identity)) = connection_data {
                    if let Some(ltk) = self.security_manager.get_peer_long_term_key(&identity) {
                        if let Some(LeConnRole::Central) = role {
                            host.async_command(LeEnableEncryption::new(
                                handle,
                                [0; 8],
                                0,
                                ltk.to_le_bytes(),
...
impl<P> DisconnectRequest<'_, '_, P> {
    ...
    pub fn confirm(self) {
        let mut state = self.state.borrow_mut();
        let storage = &mut state.connections[self.index];
    ...
    pub fn confirm_absent(self) {
        let mut state = self.state.borrow_mut();
        state.connections[self.index].state = ConnectionState::Disconnected;
    }
}
...
pub(crate) mod tests {
    ...
    #[test]
    fn absent_controller_handle_releases_the_host_slot() {
        let mgr = setup();
        ...
        let Poll::Ready(connection) = mgr.poll_accept(LeConnRole::Central, &[], None) else {
            panic!("expected connection to be accepted");
        };
        connection.disconnect();

        let Poll::Ready(request) = mgr.poll_disconnecting(None) else {
            panic!("expected disconnection request");
        };
    ...
    #[test]
    fn connecting_handle_can_be_quarantined_before_acceptance() {
        let mgr = setup();
        ...
        mgr.request_handle_disconnect(handle, DisconnectReason::RemoteUserTerminatedConn);

        assert!(mgr.poll_accept(LeConnRole::Central, &[], None).is_pending());
        let Poll::Ready(request) = mgr.poll_disconnecting(None) else {
            panic!("expected quarantined connection to request disconnection");
        };
    ...
    #[cfg(feature = "gatt")]
    #[test]
    fn gatt_client_receive_reports_disconnection() {
        let mgr = setup();
        let handle = ConnHandle::new(3);
        ...
        assert!(matches!(
            block_on(connection.next_gatt_client()),
            Err(Error::Disconnected)
        ));
    }
