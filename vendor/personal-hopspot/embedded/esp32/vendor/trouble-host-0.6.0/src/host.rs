...
use bt_hci::cmd::le::{
    LeConnUpdate, LeCreateConnCancel, LeEnableEncryption, LeLongTermKeyRequestReply,
    LeReadBufferSize, LeReadFilterAcceptListSize, LeSetAdvEnable, LeSetEventMask,
...
use bt_hci::event::le::{
    LeAdvertisingSetTerminated, LeConnectionComplete, LeConnectionUpdateComplete,
    LeDataLengthChange, LeEnhancedConnectionComplete, LeEventKind, LeEventPacket,
...
use bt_hci::param::{
    AddrKind, AdvHandle, AdvSet, BdAddr, ConnHandle, DisconnectReason, EventMask, EventMaskPage2,
    FilterDuplicates, LeConnRole, LeEventMask, Status,
};
...
use crate::types::l2cap::{
    ConnParamUpdateReq, ConnParamUpdateRes, L2capHeader, L2capSignal, L2capSignalHeader,
    L2CAP_CID_ATT, L2CAP_CID_DYN_START, L2CAP_CID_LE_U_SECURITY_MANAGER, L2CAP_CID_LE_U_SIGNAL,
};
    ...
    P: PacketPool,
{
    ...
    fn handle_acl(
        ...
        let (header, pdu) = match acl.boundary_flag() {
            AclPacketBoundary::FirstFlushable => {
                ...
                if header.channel == L2CAP_CID_LE_U_SIGNAL {
                    ...
                    self.channels
                        .signal(acl.handle(), data, &self.connections)?;
                    return Ok(());
                }
                        ...
                        self.connections.reassembly(acl.handle(), |p| {
                            let r = if !p.in_progress() {
                                ...
                                let Some(packet) = P::allocate() else {
                                    warn!(
                                        "[host] no memory for packets on channel {}",
                                        header.channel
                                    );
        ...
        match header.channel {
            L2CAP_CID_ATT => {
                ...
                if let Ok(att::Att::Client(AttClient::Request(att::AttReq::ExchangeMtu { mtu }))) =
                    a
                {
                    let mtu = self.connections.exchange_att_mtu(acl.handle(), mtu);

                    let rsp =
                        att::Att::Server(AttServer::Response(att::AttRsp::ExchangeMtu { mtu }));
                ...
                } else if let Ok(att::Att::Server(AttServer::Response(
                    att::AttRsp::ExchangeMtu { mtu },
                ))) = a
                {
                    ...
                    #[cfg(not(feature = "gatt"))]
                    {
                        if let Ok(att::Att::Client(_)) = a {
                            ...
                            self.connections
                                .try_outbound(acl.handle(), Pdu::new(packet, len))?;
                            warn!(
                                "[host] got attribute request but 'gatt' feature is not enabled."
                            );
                            return Ok(());
                        } else {
            ...
            other if other >= L2CAP_CID_DYN_START => {
                match self.channels.dispatch(header.channel, pdu) {
                    Ok(_) => {}
                    Err(e) => {
    ...
    pub(crate) async fn l2cap(
        ...
        let grant = poll_fn(|cx| {
            self.connections
                .poll_request_to_send(handle, n_acl as usize, Some(cx))
        })
        .await?;
    ...
    pub(crate) fn try_l2cap(
        ...
        let grant = match self
            .connections
            .poll_request_to_send(handle, n_acl as usize, None)
        {
            Poll::Ready(res) => res?,
            Poll::Pending => {
...
impl<'d, C: Controller, P: PacketPool> RxRunner<'d, C, P> {
        ...
        C: ControllerCmdSync<Disconnect>,
    {
        ...
        loop {
            ...
            match result {
                Ok(ControllerToHostPacket::Acl(_)) if host.initialized.try_get().is_none() => {}
                ...
                Ok(ControllerToHostPacket::Event(event)) => {
                    match event.kind {
                        EventKind::Le => {
                            ...
                            match event.kind {
                                LeEventKind::LeConnectionComplete => {
                                    let e = unwrap!(LeConnectionComplete::from_hci_bytes_complete(
                                        event.data
                                    ));
                                    let initialized = host.initialized.try_get().is_some();
                                    if !initialized {
                                        continue;
                                    }
                                    let unsolicited = e.role == LeConnRole::Central
                                        && !host.connect_command_state.is_active();
                                    if !host.handle_connection(
                                        ...
                                        e.role,
                                        ConnParams {
                                            conn_interval: Duration::from_micros(
                                                e.conn_interval.as_micros(),
                                            ),
                                ...
                                LeEventKind::LeEnhancedConnectionComplete => {
                                    let e = unwrap!(
                                        LeEnhancedConnectionComplete::from_hci_bytes_complete(
                                            event.data
                                        )
                                    );
                                    let initialized = host.initialized.try_get().is_some();
                                ...
                                LeEventKind::LeAdvertisingSetTerminated => {
                                    let set = unwrap!(
                                        LeAdvertisingSetTerminated::from_hci_bytes_complete(
                                            event.data
                                        )
                                    );
                                ...
                                LeEventKind::LeExtendedAdvertisingReport => {
                                    ...
                                    {
                                        let data = unwrap!(
                                            LeExtendedAdvertisingReport::from_hci_bytes_complete(
                                                event.data
                                            )
                                        );
                                ...
                                LeEventKind::LeAdvertisingReport => {
                                    ...
                                    {
                                        let data =
                                            unwrap!(LeAdvertisingReport::from_hci_bytes_complete(
                                                event.data
                                            ));
                                ...
                                LeEventKind::LePhyUpdateComplete => {
                                    let event = unwrap!(
                                        LePhyUpdateComplete::from_hci_bytes_complete(event.data)
                                    );
                                    if let Err(e) = event.status.to_result() {
                                        warn!(
                                            "[host] error updating phy for {:?}: {:?}",
                                            event.handle, e
                                        );
                                    } else {
                                ...
                                LeEventKind::LeConnectionUpdateComplete => {
                                    let event = unwrap!(
                                        LeConnectionUpdateComplete::from_hci_bytes_complete(
                                            event.data
                                        )
                                    );
                                    if let Err(e) = event.status.to_result() {
                                ...
                                LeEventKind::LeDataLengthChange => {
                                    let event = unwrap!(
                                        LeDataLengthChange::from_hci_bytes_complete(event.data)
                                    );
                                    let _ = host.connections.post_handle_event(
                                        event.handle,
                                ...
                                LeEventKind::LeRemoteConnectionParameterRequest => {
                                    let event = unwrap!(
                                        LeRemoteConnectionParameterRequest::from_hci_bytes_complete(
                                            event.data
                                        )
                                    );
                        ...
                        EventKind::DisconnectionComplete => {
                            let e =
                                unwrap!(DisconnectionComplete::from_hci_bytes_complete(event.data));
                            let handle = e.handle;
                            let reason = if let Err(e) = e.status.to_result() {
                                info!(
                                    "[host] disconnection event on handle {}, status: {:?}",
                                    handle.raw(),
                                    e
                                );
                                None
                        ...
                        EventKind::NumberOfCompletedPackets => {
                            let c = unwrap!(NumberOfCompletedPackets::from_hci_bytes_complete(
                                event.data
                            ));
                            ...
                            for entry in c.completed_packets.iter() {
                                match (entry.handle(), entry.num_completed_packets()) {
                                    (Ok(handle), Ok(completed)) => {
                                        let _ = host
                                            .connections
                                            .confirm_sent(handle, completed as usize);
                                    }
                                    (Ok(handle), Err(e)) => {
...
impl<'d, C: Controller, P: PacketPool> ControlRunner<'d, C, P> {
            ...
            + ControllerCmdSync<ReadBdAddr>,
    {
        ...
        let ret = LeReadFilterAcceptListSize::new()
            .exec(&host.controller)
            .await?;
...

        {
            ...
            if let Err(_e) = HostBufferSize::new(ACL_LEN, 0, ACL_N, 0)
                .exec(&host.controller)
                .await
            {
            ...
            .await
            {
                Either3::First(request) => {
                    ...
                    match host
                        ...
                        .await
                    {
                        Ok(_) => request.confirm(),
                        Err(BleHostError::BleHost(Error::Hci(
                            bt_hci::param::Error::UNKNOWN_CONN_IDENTIFIER,
                        ))) => request.confirm_absent(),
                        Err(e) => {
                            return Err(e);
                        }
                    }
                ...
                Either3::Third(states) => match states {
                    ...
                    Either4::Fourth(request) => {
                        #[cfg(feature = "security")]
                        {
                            ...
                            host.connections
                                .handle_security_event(host, event_data)
                                .await?;
                        }
                    }
                },
