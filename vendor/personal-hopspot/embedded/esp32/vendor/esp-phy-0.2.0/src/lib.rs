...
impl PhyState {
    ...
    fn calibrate(&mut self) {
        ...
        #[cfg(all(phy_combo_module, not(esp32c5)))]
        unsafe {
...
/// Enables the shared PHY and explicitly enables Wi-Fi reception.
pub fn enable_phy_with_wifi_rx() {
    core::mem::forget(enable_phy());
    set_wifi_rx_enabled(true);
}

/// Disables Wi-Fi reception before disabling the shared PHY.
pub fn disable_phy_with_wifi_rx() {
    set_wifi_rx_enabled(false);
    disable_phy();
}

/// Reasserts Wi-Fi reception without changing shared-PHY ownership.
pub fn reassert_wifi_rx_enabled() {
    set_wifi_rx_enabled(true);
}
...
fn set_wifi_rx_enabled(enabled: bool) {
    ...
    #[cfg(any(not(phy_combo_module), esp32c5))]
    let _ = enabled;
}
