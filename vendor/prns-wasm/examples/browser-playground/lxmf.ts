...
export type LxmfDeliveryProfile = {
  readonly displayName: string;
  readonly registration: RegisterSingleDestinationOptions;
};

export type LxmfDeliveryProfileFailure = Tagged<
  "LxmfDisplayNameTooLong",
  { readonly actual: number; readonly maximum: number }
>;

export type LxmfDeliveryProfileOutcome =
  | Tagged<"Prepared", LxmfDeliveryProfile>
  | LxmfDeliveryProfileFailure;

export const BROWSER_PLAYGROUND_LXMF_DELIVERY = prepareLxmfDeliveryProfile(
  LXMF_DELIVERY_DISPLAY_NAME,
);

function prepareLxmfDeliveryProfile(
  displayName: string,
): LxmfDeliveryProfileOutcome {
  ...
  const encoded = new Uint8Array(
    MESSAGEPACK_BIN8_HEADER_LENGTH +
